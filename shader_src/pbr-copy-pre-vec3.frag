...
LTC_evaluate(vec3 N, vec3 V, vec3 P, mat3 Minv, vec4 viewspace_points[MAX_UNCLIPPED_NGON], int viewspace_points_n, bool double_sided)
{
    ...
    vec3 light_normal = cross(viewspace_points[1] - viewspace_points[0], viewspace_points[3] - viewspace_points[0]);
    ...
    float sum = integrate_lambertian_hemisphere(points_o, n);
    if (double_sided)
    {
        sum = abs(sum);
    }
...
main()
{
    ...
    */
    ...
    sum_arealight_radiance += al.color_rgb_intensity_a.a * al.color_rgb_intensity_a.rgb
        * LTC_evaluate(N, V, frag_position_viewspace, Minv, al.points_viewspace, 4, al.is_double_sided == 1);
    // for (int i = 0; i < al.n; ++i)
    // {
    //     if (distance(al.points_viewspace[i].xy, frag_position_viewspace.xy) < 0.2)
    //         sum_arealight_radiance += vec3(1.0, 0.0, 0.0);
    // }
    /////////////////////////////////////////////////////////

    // Add ambient light
    vec3 ambient_color = vec3(0.1);//vec3(0.01);
    ...
    // vec3 final_linear_color = sum_arealight_radiance;
